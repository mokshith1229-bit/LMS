...
export default function PresenterConsole() {
  ...
  useEffect(() => {
    ...
    return () => channel.close();
  }, [currentSlide, slideDir, mode, activePoll, chartData, presentationResponseCount, pollTimerActive, timeLeft, pollRevealed, summaryPage, currentQuestionIndex, id]);

  // Disable auto-fullscreen for PresenterConsole
  const toggleFullscreen = () => {
    // Intentionally disabled in Presenter Console
    toast("Please use Presentation View on the extended display to go fullscreen.", { icon: 'ℹ️' });
  };
